...
final class WebRTCManager: NSObject, ObservableObject {
   ...
   private func calculateLadderForDevice(_ device: AVCaptureDevice) {
        ...
        AVCaptureDevice.requestAccess(for: .video) { granted in
                    DispatchQueue.main.async {
                        ...
                        if wantFront {
                            ...
                            // 🔥 后置优先使用超广角摄像头
                            initialCamera = self.getBackCamera(preferUltraWide: true) ?? devices.first
    ...
    private func startCaptureWithDevice(_ device: AVCaptureDevice, width: Int, height: Int, fps: Int) {
        ...
        guard let best = allFormats.sorted(by: { f0, f1 in
            ...
            // 🔥 分辨率相同时，FOV 最大的优先（关键！）
            let fov0 = f0.videoFieldOfView
            let fov1 = f1.videoFieldOfView
            if abs(fov0 - fov1) > 0.1 { return fov0 > fov1 }  // FOV 更大的优先
        ...
        let selectedFov = best.videoFieldOfView
        ...
        print("   🎯 选中格式: \(dims.width)x\(dims.height) 最大FPS=\(maxFps) FOV=\(String(format: "%.1f", selectedFov))°")
   ...
   private let ADAPT_MIN_INTERVAL_SEC: TimeInterval = 8
    ...
    // 🔥 是否使用超广角摄像头（后置专用）
    var useUltraWideCamera: Bool = true  // 默认开启超广角
    
    /// 获取后置摄像头（优先超广角）
    private func getBackCamera(preferUltraWide: Bool = true) -> AVCaptureDevice? {
        // 先尝试从 DiscoverySession 获取超广角
        if preferUltraWide && useUltraWideCamera {
            let discoverySession = AVCaptureDevice.DiscoverySession(
            ...
            if let ultraWide = discoverySession.devices.first {
                print("🌐 使用超广角摄像头: \(ultraWide.localizedName)")
                return ultraWide
            }
        }
    ...
    // 🔥 固定采集分辨率（始终使用最高分辨率采集，通过 scaleResolutionDownBy 缩放输出）
    ...
    func startPublish(initialProfile: LadderProfile? = nil) {
        ...
        if let pushTrack = localVideoTrack, capturer != nil {
            ...
            if wantFront {
                ...
                // 🔥 后置优先使用超广角摄像头
                initialCamera = getBackCamera(preferUltraWide: true) ?? devices.first
    ...
    // - 主摄像头 (builtInWideAngleCamera) 支持 minZoom ≈ 0.5（通过切换到超广角镜头）
    // - 超广角摄像头 (builtInUltraWideCamera) 的 minZoom = 1.0（已经是最广，无法再缩小）
    // - zoom = 1.0 表示当前镜头的原始视野
    ...
    func setZoom(_ factor: CGFloat) {
        ...
        let isUltraWide = dev.deviceType == .builtInUltraWideCamera
        
        // 🔥 打印当前格式的 FOV（帮助诊断视野问题）
        let currentFOV = dev.activeFormat.videoFieldOfView
        let dims = CMVideoFormatDescriptionGetDimensions(dev.activeFormat.formatDescription)
        print("🔍 [setZoom] 设备: \(position) (\(deviceType)), 当前格式: \(dims.width)x\(dims.height) FOV=\(String(format: "%.1f", currentFOV))°")
        
        do {
            try dev.lockForConfiguration()
            
            // 🔥 强制测试：最小值设为 0.5，看设备是否支持
            let deviceMinZoom = dev.minAvailableVideoZoomFactor
            let forceMinZoom: CGFloat = 0.5  // 🔥 强制 0.5 测试
            let maxZoom = dev.activeFormat.videoMaxZoomFactor
            let currentZoom = dev.videoZoomFactor
            
            print("🔍 [setZoom] 设备报告 minZoom=\(deviceMinZoom), 强制测试 minZoom=\(forceMinZoom)")
            
            // 🔥 直接使用 0.5 作为最小值，看是否会报错
            let safe = max(forceMinZoom, min(factor, maxZoom))
            
            print("🔍 [setZoom] 当前zoom=\(currentZoom), 请求=\(factor), 范围=\(forceMinZoom)~\(maxZoom), 最终=\(safe)")
            ...
            if abs(verifyZoom - safe) > 0.01 {
                ...
                print("✅ [setZoom] 设置成功: zoom=\(verifyZoom)")
            }
        } catch {
    ...
    func toggleCamera() {
        ...
        if newPos == .back {
            dev = getBackCamera(preferUltraWide: true)
        } else {
            dev = RTCCameraVideoCapturer.captureDevices().first(where: { $0.position == newPos })
        }
        ...
        guard let best = allFormats.sorted(by: { f0, f1 in
            ...
            let fov0 = f0.videoFieldOfView
            let fov1 = f1.videoFieldOfView
            if abs(fov0 - fov1) > 0.1 { return fov0 > fov1 }
    ...
    func recapture(width: Int, height: Int, fps: Int) {
        ...
        let devOpt: AVCaptureDevice? = {
                ...
                // 🔥 后置优先使用超广角摄像头
                if let ultraWide = self.getBackCamera(preferUltraWide: true) {
                    return ultraWide
                }
                let devices = RTCCameraVideoCapturer.captureDevices()
