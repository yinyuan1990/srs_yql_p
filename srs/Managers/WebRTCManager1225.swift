...
final class WebRTCManager: NSObject, ObservableObject {
   ...
   private func calculateLadderForDevice(_ device: AVCaptureDevice) {
        // 🔥 4档配置（3种缩放比例）：所有档位 maxPushFps=60
        // scale=1 (1920x1080): 4K档
        // scale=1.5 (1280x720): 超清档
        // scale=2 (960x540): 高清档 + 标清档（通过码率区分）
        
        if device.position == .back {
            // 🎥 后置摄像头：4档固定（采集1080p@240fps，不同档位不同推送FPS上限）
            currentLadder = [
                .p4k:      LadderPreset(width: 1920, height: 1080, fps: 240, maxKbps: 4500, maxPushFps: 60),   // scale=1, 最高60fps
                .ultra:    LadderPreset(width: 1280, height: 720,  fps: 240, maxKbps: 3000, maxPushFps: 60),   // scale=1.5, 最高60fps
                .high:     LadderPreset(width: 960,  height: 540,  fps: 240, maxKbps: 2000, maxPushFps: 60),   // scale=2, 最高60fps
                .standard: LadderPreset(width: 960,  height: 540,  fps: 240, maxKbps: 1500, maxPushFps: 60)    // scale=2, 最高60fps
            ]
            print("📐 后置摄像头 - 4档配置（1080p采集，缩放输出）：")
            print("   4K    = 1920x1080 (scale=1)   → 3500kbps, maxPushFps=60")
            print("   超清  = 1280x720  (scale=1.5) → 3000kbps, maxPushFps=60")
            print("   高清  = 960x540   (scale=2)   → 2000kbps, maxPushFps=60")
            print("   标清  = 960x540   (scale=2)   → 1500kbps, maxPushFps=60")
        } else {
            // 📱 前置摄像头：4档固定（采集1080p@120fps，不同档位不同推送FPS上限）
            currentLadder = [
                .p4k:      LadderPreset(width: 1920, height: 1080, fps: 120, maxKbps: 4500, maxPushFps: 60),   // scale=1, 最高60fps
                .ultra:    LadderPreset(width: 1280, height: 720,  fps: 120, maxKbps: 3000, maxPushFps: 60),   // scale=1.5, 最高60fps
                .high:     LadderPreset(width: 960,  height: 540,  fps: 120, maxKbps: 1500, maxPushFps: 60),   // scale=2, 最高60fps
                .standard: LadderPreset(width: 960,  height: 540,  fps: 120, maxKbps: 1200, maxPushFps: 60)    // scale=2, 最高60fps
            ]
            print("📐 前置摄像头 - 4档配置（1080p采集，缩放输出）：")
            print("   4K    = 1920x1080 (scale=1)   → 3200kbps, maxPushFps=60")
            print("   超清  = 1280x720  (scale=1.5) → 2500kbps, maxPushFps=60")
            print("   高清  = 960x540   (scale=2)   → 1500kbps, maxPushFps=60")
            print("   标清  = 960x540   (scale=2)   → 1200kbps, maxPushFps=60")
        }
    }
    ...
    /// 根据后端百分比计算目标采集FPS
    /// - Parameter percentage: 后端下发的 0-100 值
    /// - Returns: 实际采集FPS（前置60-120，后置60-240）
    private func calculateTargetCaptureFPS(percentage: Int, isBackCamera: Bool) -> Int {
        let clamped = max(0, min(100, percentage))
        if isBackCamera {
            // 后置: 60 + (cjfps/100) * 180 = 60~240fps
            return 60 + Int(Double(clamped) * 1.8)
        } else {
            // 前置: 60 + (cjfps/100) * 60 = 60~120fps
            return 60 + Int(Double(clamped) * 0.6)
        }
    }
    
    /// 设置采集FPS百分比（后端下发）
    /// - Parameter percentage: 0-100
    func setCaptureFrameRate(percentage: Int) {
        ...
        if let input = capturer?.captureSession.inputs.first as? AVCaptureDeviceInput {
            isBackCamera = (input.device.position == .back)
        } else {
            // 默认后置
            isBackCamera = true
        }
        let oldTargetFps = calculateTargetCaptureFPS(percentage: oldPercentage, isBackCamera: isBackCamera)
        let newTargetFps = calculateTargetCaptureFPS(percentage: cjfpsPercentage, isBackCamera: isBackCamera)
        
        print("📹 [采集FPS] 百分比: \(oldPercentage)% → \(cjfpsPercentage)%")
        print("   \(isBackCamera ? "后置" : "前置")摄像头: 目标FPS \(oldTargetFps)fps → \(newTargetFps)fps")
        
        // 如果目标FPS有变化，需要重新配置采集
        if oldTargetFps != newTargetFps {
            applyCaptureFPSChange(targetFps: newTargetFps)
        }
    }
    
    /// 应用采集FPS变化（重新配置摄像头采集帧率）
    private func applyCaptureFPSChange(targetFps: Int) {
        guard let capturer = self.capturer,
              let device = getCurrentCaptureDevice() else {
            print("⚠️ [applyCaptureFPSChange] capturer 或 device 不存在")
            return
        }
        
        // 获取当前使用的 format
        guard let currentFormat = device.activeFormat as AVCaptureDevice.Format? else {
            print("⚠️ [applyCaptureFPSChange] 无法获取当前 format")
            return
        }
        
        // 检查目标FPS是否在格式支持范围内
        let maxSupportedFps = Int(currentFormat.videoSupportedFrameRateRanges.map(\.maxFrameRate).max() ?? 30)
        let finalFps = min(targetFps, maxSupportedFps)
        
        print("📹 [applyCaptureFPSChange] 目标FPS=\(targetFps), 格式最大=\(maxSupportedFps), 最终=\(finalFps)")
        
        // 更新记录的采集FPS
        let oldCaptureFps = currentCaptureFPS
        currentCaptureFPS = finalFps
        
        // 🔥 使用回调形式确保 stopCapture 完成后再开始
        capturer.stopCapture { [weak self] in
            guard let self = self else { return }
            
            self.configureCameraAutoModes(device)
            self.capturer.startCapture(with: device, format: currentFormat, fps: finalFps)
            
            print("🔄 [applyCaptureFPSChange] 采集FPS已更新: \(oldCaptureFps)fps → \(finalFps)fps")
            
            // 🔥 采集FPS变化后，节流逻辑自动适应（FrameThrottler 内部会检测 detectedCaptureFps）
            // 但需要检查推送FPS是否超过新的采集FPS
            if let sendFps = self.frameThrottler?.targetSendFps, sendFps > finalFps {
                self.frameThrottler?.targetSendFps = finalFps
                self.targetOutputFPS = finalFps
                print("⚠️ 推送FPS(\(sendFps)) 超过新采集FPS(\(finalFps))，已限制")
            }
    ...
    func applyThinRemoteConfig(_ cfg: ThinRemoteConfig) {
        ...
        case "cjfps":
            ...
            if let cj = cfg.cjfps {
                let isBack: Bool
                if let input = capturer?.captureSession.inputs.first as? AVCaptureDeviceInput {
                    isBack = (input.device.position == .back)
                } else {
                    isBack = (cfg.direction != "1")  // 根据配置判断
                }
                let targetFps = calculateTargetCaptureFPS(percentage: cj, isBackCamera: isBack)
                print("📹 [采集FPS设置] 后端: \(cj)% → \(isBack ? "后置" : "前置") 目标: \(targetFps)fps")
    ...
    func applyThinRemoteConfigInit(_ cfg: ThinRemoteConfig) {
            ...
            // 4.5) 🔥 采集FPS（0-100百分比）
            if let cj = cfg.cjfps {
                cjfpsPercentage = max(0, min(100, cj))
                let isBack = (cfg.direction != "1")  // -1=后置, 1=前置
                let targetFps = calculateTargetCaptureFPS(percentage: cj, isBackCamera: isBack)
                print("📹 [采集FPS设置-初始化] 后端: \(cj)% → \(isBack ? "后置" : "前置") 目标: \(targetFps)fps")
                // 注意：初始化时不立即应用，等 startCaptureWithDevice 时使用
                }
    ...
    func startPreviewIfNeeded(initialProfile: LadderProfile? = nil) {
        ...
        AVCaptureDevice.requestAccess(for: .video) { granted in
                    DispatchQueue.main.async {
                        ...
                        // 🔥🔥🔥 完整格式诊断：按分辨率分组，对比同分辨率不同FPS的FOV差异 🔥🔥🔥
                        print("\n")
                        print("╔══════════════════════════════════════════════════════════════════╗")
                        print("║              📐 完整摄像头格式诊断（FOV 对比）                    ║")
                        print("╚══════════════════════════════════════════════════════════════════╝")
                        
                        // 辅助函数：打印某个摄像头的所有格式（按分辨率分组）
                        func printFormatsGrouped(camera: AVCaptureDevice, name: String) {
                            let formats = RTCCameraVideoCapturer.supportedFormats(for: camera)
                            
                            // 只看横屏格式（width > height）
                            let landscapeFormats = formats.filter {
                                let dims = CMVideoFormatDescriptionGetDimensions($0.formatDescription)
                                return dims.width > dims.height
                            }
                            
                            print("\n🎥 【\(name)】横屏格式 (共\(landscapeFormats.count)个)")
                            print("┌─────────────────┬────────┬─────────┬─────────┬──────────┐")
                            print("│    分辨率       │ 画幅比 │ 最大FPS │  FOV°   │  Binned  │")
                            print("├─────────────────┼────────┼─────────┼─────────┼──────────┤")
                            
                            // 按分辨率分组
                            var groupedByRes: [String: [(fmt: AVCaptureDevice.Format, idx: Int)]] = [:]
                            for (idx, fmt) in landscapeFormats.enumerated() {
                                let dims = CMVideoFormatDescriptionGetDimensions(fmt.formatDescription)
                                let key = "\(dims.width)x\(dims.height)"
                                if groupedByRes[key] == nil {
                                    groupedByRes[key] = []
                                }
                                groupedByRes[key]?.append((fmt, idx))
                            }
                            
                            // 按分辨率排序（从大到小）
                            let sortedKeys = groupedByRes.keys.sorted { k1, k2 in
                                let w1 = Int(k1.split(separator: "x").first ?? "0") ?? 0
                                let w2 = Int(k2.split(separator: "x").first ?? "0") ?? 0
                                return w1 > w2
                            }
                            
                            var maxFovFormat: (res: String, fps: Int, fov: Float) = ("", 0, 0)
                            
                            for resKey in sortedKeys {
                                guard let fmts = groupedByRes[resKey] else { continue }
                                
                                // 按 FPS 排序
                                let sortedFmts = fmts.sorted { a, b in
                                    let fps1 = Int(a.fmt.videoSupportedFrameRateRanges.map(\.maxFrameRate).max() ?? 0)
                                    let fps2 = Int(b.fmt.videoSupportedFrameRateRanges.map(\.maxFrameRate).max() ?? 0)
                                    return fps1 > fps2
                                }
                                
                                for (i, item) in sortedFmts.enumerated() {
                                    let dims = CMVideoFormatDescriptionGetDimensions(item.fmt.formatDescription)
                                    let maxFps = Int(item.fmt.videoSupportedFrameRateRanges.map(\.maxFrameRate).max() ?? 0)
                                    let fov = item.fmt.videoFieldOfView
                                    let binned = item.fmt.isVideoBinned ? "Yes" : "No"
                                    ...
                                    // 记录最大FOV
                                    if fov > maxFovFormat.fov {
                                        maxFovFormat = (resKey, maxFps, fov)
                                    }
                                    
                                    // 第一行显示分辨率，后续行缩进
                                    let resDisplay = i == 0 ? resKey : "  ↳"
                                    print("│ \(resDisplay.padding(toLength: 15, withPad: " ", startingAt: 0)) │ \(ratioStr.padding(toLength: 6, withPad: " ", startingAt: 0)) │ \(String(maxFps).padding(toLength: 7, withPad: " ", startingAt: 0)) │ \(String(format: "%6.1f", fov))° │ \(binned.padding(toLength: 8, withPad: " ", startingAt: 0)) │")
                                }
                            }
                            
                            print("└─────────────────┴────────┴─────────┴─────────┴──────────┘")
                            print("🏆 最大FOV格式: \(maxFovFormat.res) @\(maxFovFormat.fps)fps → FOV=\(String(format: "%.1f", maxFovFormat.fov))°")
                            
                            // 🔥 重点对比：同分辨率不同FPS的FOV差异
                            print("\n📊 【关键对比】同分辨率不同FPS的FOV差异:")
                            for resKey in ["1920x1080", "1280x720", "1440x1080", "1280x960"] {
                                guard let fmts = groupedByRes[resKey], fmts.count > 1 else { continue }
                                ...
                                var fovList: [(fps: Int, fov: Float)] = []
                                for item in sortedFmts {
                                    let maxFps = Int(item.fmt.videoSupportedFrameRateRanges.map(\.maxFrameRate).max() ?? 0)
                                    let fov = item.fmt.videoFieldOfView
                                    // 去重
                                    if !fovList.contains(where: { $0.fps == maxFps && abs($0.fov - fov) < 0.1 }) {
                                        fovList.append((maxFps, fov))
                                    }
                                }
                                
                                if fovList.count > 1 {
                                    let fovStrs = fovList.map { "@\($0.fps)fps=\(String(format: "%.1f", $0.fov))°" }
                                    print("   \(resKey): \(fovStrs.joined(separator: " vs "))")
                                    
                                    // 如果 FOV 有明显差异，标注
                                    if let maxFov = fovList.max(by: { $0.fov < $1.fov }),
                                       let minFov = fovList.min(by: { $0.fov < $1.fov }),
                                       maxFov.fov - minFov.fov > 5 {
                                        print("      ⚠️ 警告: 高帧率(\(minFov.fps)fps)比低帧率(\(maxFov.fps)fps)视野窄 \(String(format: "%.1f", maxFov.fov - minFov.fov))°!")
                                    }
                                }
                            }
                        }
                        ...
                        if let frontCamera = devices.first(where: { $0.position == .front }) {
                            printFormatsGrouped(camera: frontCamera, name: "前置摄像头")
                        }
                        
                        // 打印后置主摄
                        if let backCamera = devices.first(where: { $0.position == .back && $0.deviceType == .builtInWideAngleCamera }) {
                            printFormatsGrouped(camera: backCamera, name: "后置主摄(WideAngle)")
                        } else if let backCamera = devices.first(where: { $0.position == .back }) {
                            printFormatsGrouped(camera: backCamera, name: "后置摄像头")
                        }
                        
                        // 打印超广角（如果有）
                        if let ultraWide = devices.first(where: { $0.deviceType == .builtInUltraWideCamera }) {
                            printFormatsGrouped(camera: ultraWide, name: "后置超广角(UltraWide)")
                        }
                        ...
                        // ✅ 直接使用初始选择的摄像头启动（根据前后置选择分辨率）
                        if let camera = initialCamera {
                            let w = camera.position == .front ? self.fixedCaptureWidthFront : self.fixedCaptureWidthBack
                            let h = camera.position == .front ? self.fixedCaptureHeightFront : self.fixedCaptureHeightBack
                            self.startCaptureWithDevice(camera, width: w, height: h, fps: 60)
                        }
    ...
    private func startCaptureWithDevice(_ device: AVCaptureDevice, width: Int, height: Int, fps: Int) {
        ...
        if !matchingFormats.isEmpty {
            print("   找到\(matchingFormats.count)个匹配\(targetWidth)x\(targetHeight)的格式")
        }
        
        // 分辨率优先：先选最接近 1920x1080 的，分辨率相同时选最高FPS的
        guard let best = allFormats.sorted(by: { f0, f1 in
        ...
        // 🔥 根据后端 cjfps 百分比计算目标采集FPS
        let isBackCamera = (device.position == .back)
        let targetCaptureFps = calculateTargetCaptureFPS(percentage: cjfpsPercentage, isBackCamera: isBackCamera)
        
        // 🔥 采集FPS = min(目标FPS, 设备最大FPS)
        let useFps = min(targetCaptureFps, maxFps)
        currentCaptureFPS = useFps  // 记录采集FPS
        
        print("📹 [采集FPS计算] cjfps=\(cjfpsPercentage)%, \(isBackCamera ? "后置" : "前置")")
        print("   目标=\(targetCaptureFps)fps, 设备最大=\(maxFps)fps → 实际采集=\(useFps)fps")
        ...
        configureCameraAutoModes(device)
    ...
    // 前置: 60 + (cjfps/100) * 60 = 60~120fps
    // 后置: 60 + (cjfps/100) * 180 = 60~240fps
    private var cjfpsPercentage: Int = 100  // 默认100%，即使用设备最大FPS
   ...
   private let ADAPT_MIN_INTERVAL_SEC: TimeInterval = 8
    ...
    // 🔥 固定采集分辨率
    private let fixedCaptureWidthBack: Int = 1920
    private let fixedCaptureHeightBack: Int = 1080
    private let fixedCaptureWidthFront: Int = 1920
    private let fixedCaptureHeightFront: Int = 1440  // 4:3 画幅
    
    // 动态获取当前采集分辨率
    private var fixedCaptureWidth: Int {
        if let devInput = capturer?.captureSession.inputs.first as? AVCaptureDeviceInput {
            return devInput.device.position == .front ? fixedCaptureWidthFront : fixedCaptureWidthBack
        }
        return fixedCaptureWidthBack
    }
    private var fixedCaptureHeight: Int {
        if let devInput = capturer?.captureSession.inputs.first as? AVCaptureDeviceInput {
            return devInput.device.position == .front ? fixedCaptureHeightFront : fixedCaptureHeightBack
        }
        return fixedCaptureHeightBack
    }
    ...
    func applyProfileBitrateOnly(_ p: LadderProfile) {
            ...
            print("   🔥 热切换完成: 输出=\(outputWidth)x\(outputHeight), 最高\(maxPushFps)fps, 码率=\(targetKbps)kbps")
    ...
    func startPublish(initialProfile: LadderProfile? = nil) {
        ...
        if let pushTrack = localVideoTrack, capturer != nil {
            ...
            // ✅ 直接使用初始选择的摄像头启动（根据前后置选择分辨率）
            if let camera = initialCamera {
                let w = camera.position == .front ? fixedCaptureWidthFront : fixedCaptureWidthBack
                let h = camera.position == .front ? fixedCaptureHeightFront : fixedCaptureHeightBack
                startCaptureWithDevice(camera, width: w, height: h, fps: 60)
            }
        }
    ...
    func toggleCamera() {
        ...
        // 🔥 根据新摄像头位置选择采集分辨率
        let targetWidth = newPos == .front ? fixedCaptureWidthFront : fixedCaptureWidthBack
        let targetHeight = newPos == .front ? fixedCaptureHeightFront : fixedCaptureHeightBack
        
        print("🔄 [toggleCamera] 固定采集分辨率: \(targetWidth)x\(targetHeight) (\(newPos == .front ? "前置" : "后置"))")
        ...
        // 🔥 根据后端 cjfps 百分比计算目标采集FPS
        let isBackCamera = (dev.position == .back)
        let targetCaptureFps = calculateTargetCaptureFPS(percentage: cjfpsPercentage, isBackCamera: isBackCamera)
       ...
       currentCaptureFPS = useFps  // 记录采集FPS
        ...
        print("📹 [toggleCamera] cjfps=\(cjfpsPercentage)%, \(isBackCamera ? "后置" : "前置")")
        print("   目标=\(targetCaptureFps)fps, 设备最大=\(maxFps)fps → 实际采集=\(useFps)fps")
    ...
    func recapture(width: Int, height: Int, fps: Int) {
        ...
        // 🔥 根据摄像头位置选择采集分辨率
        let targetWidth = dev.position == .front ? fixedCaptureWidthFront : fixedCaptureWidthBack
        let targetHeight = dev.position == .front ? fixedCaptureHeightFront : fixedCaptureHeightBack
        ...
        statsTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { [weak self] _ in
                ...
                pc.statistics { report in
                    ...
                    DispatchQueue.main.async {
                        ...
                        print("🔍 [FPS链路] cjfps=\(self.cjfpsPercentage)% 采集=\(captureFps) → 节流目标=\(targetFps) → 本地推送=\(localSendFps) → WebRTC实际=\(webrtcSendFps)")
