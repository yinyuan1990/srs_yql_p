...
final class FrameThrottler: NSObject, RTCVideoCapturerDelegate {
    ...
    // 🔥 预览旋转角度（可切换横竖屏预览）
    @Published var previewRotation: RTCVideoRotation = ._90  // 默认竖屏
    
    // 🔥 发送到预览（固定60fps）- 保持原始方向
    private func sendPreviewFrame(_ capturer: RTCVideoCapturer, videoFrame: RTCVideoFrame) {
        previewSentCounter += 1
        // ✅ 不旋转，保持原始方向
        previewDelegate?.capturer(capturer, didCapture: videoFrame)
    }
    
    // 🔥 发送到推送（受后端fps控制）- 保持原始方向
    ...
    // 🔥 切换预览方向（横屏/竖屏）
    func togglePreviewOrientation() {
        if previewRotation == ._90 {
            previewRotation = ._0   // 切换到横屏
            print("🔄 预览切换: 竖屏 → 横屏 (推流仍为9:16竖向)")
        } else {
            previewRotation = ._90  // 切换到竖屏
            print("🔄 预览切换: 横屏 → 竖屏 (推流仍为9:16竖向)")
        }
    }
    
    // 🔥 获取当前预览是否为竖屏
    var isPreviewPortrait: Bool {
        return previewRotation == ._90 || previewRotation == ._270
    }
...
final class WebRTCManager: NSObject, ObservableObject {
   ...
   private func calculateLadderForDevice(_ device: AVCaptureDevice) {
        ...
        if device.position == .back {
            // 🎥 后置摄像头：4档固定（采集1280x960@240fps 4:3，横屏锁定，缩放输出）
            currentLadder = [
                .p4k:      LadderPreset(width: 1280, height: 960, fps: 240, maxKbps: 4000, maxPushFps: 60),   // scale=1, 4:3
                .ultra:    LadderPreset(width: 960,  height: 720, fps: 240, maxKbps: 2500, maxPushFps: 60),   // scale=1.33, 4:3
                .high:     LadderPreset(width: 640,  height: 480, fps: 240, maxKbps: 1500, maxPushFps: 60),   // scale=2, 4:3
                .standard: LadderPreset(width: 480,  height: 360, fps: 240, maxKbps: 800,  maxPushFps: 60)    // scale=2.67, 4:3
            ]
            print("📐 后置摄像头 - 4档配置（1280x960@240fps 4:3采集，横屏锁定，缩放输出）：")
            print("   4K    = 1280x960 (scale=1)    → 4000kbps, maxPushFps=60")
            print("   超清  = 960x720  (scale=1.33) → 2500kbps, maxPushFps=60")
            print("   高清  = 640x480  (scale=2)    → 1500kbps, maxPushFps=60")
            print("   标清  = 480x360  (scale=2.67) → 800kbps,  maxPushFps=60")
        } else {
            // 📱 前置摄像头：4档固定（采集1280x960@120fps 4:3，横屏锁定，缩放输出）
            currentLadder = [
                .p4k:      LadderPreset(width: 1280, height: 960, fps: 120, maxKbps: 4000, maxPushFps: 60),   // scale=1, 4:3
                .ultra:    LadderPreset(width: 960,  height: 720, fps: 120, maxKbps: 2500, maxPushFps: 60),   // scale=1.33, 4:3
                .high:     LadderPreset(width: 640,  height: 480, fps: 120, maxKbps: 1500, maxPushFps: 60),   // scale=2, 4:3
                .standard: LadderPreset(width: 480,  height: 360, fps: 120, maxKbps: 800,  maxPushFps: 60)    // scale=2.67, 4:3
            ]
            print("📐 前置摄像头 - 4档配置（1280x960@120fps 4:3采集，横屏锁定，缩放输出）：")
            print("   4K    = 1280x960 (scale=1)    → 4000kbps, maxPushFps=60")
            print("   超清  = 960x720  (scale=1.33) → 2500kbps, maxPushFps=60")
            print("   高清  = 640x480  (scale=2)    → 1500kbps, maxPushFps=60")
            print("   标清  = 480x360  (scale=2.67) → 800kbps,  maxPushFps=60")
    ...
    // 🔥 预览方向控制（包装 FrameThrottler 的属性）
    @Published private(set) var isPreviewPortrait: Bool = false
    
    func togglePreviewOrientation() {
        frameThrottler?.togglePreviewOrientation()
        isPreviewPortrait = frameThrottler?.isPreviewPortrait ?? false
        print("🔄 WebRTCManager: 预览方向切换, isPortrait=\(isPreviewPortrait)")
    }
                    ...
                    DispatchQueue.main.async {
                        ...
                        if let frontCamera = devices.first(where: { $0.position == .front }) {
                            ...
                            for (index, fmt) in formats.enumerated() {
                                ...
                                let maxFps = Int(fmt.videoSupportedFrameRateRanges.map(\.maxFrameRate).max() ?? 0)
                                let isLandscape = dims.width > dims.height
                                if maxFps >= 120 && isLandscape {
                                    count120 += 1
                                    let ratio = Float(dims.width) / Float(dims.height)
                                    let ratioStr = abs(ratio - 16.0/9.0) < 0.05 ? "16:9" : (abs(ratio - 4.0/3.0) < 0.05 ? "4:3" : String(format: "%.2f", ratio))
                                    print("   [\(count120)] 横 \(dims.width)x\(dims.height) @\(maxFps)fps FOV=\(String(format: "%.1f", fmt.videoFieldOfView))° (\(ratioStr))")
                                }
                            }
                            print("   📊 共 \(count120) 个支持 120fps 的横屏格式")
                        }
                        ...
                        // ✅ 直接使用初始选择的摄像头启动（固定采集 1280×960 4:3 横屏，支持高帧率）
                        if let camera = initialCamera {
                            // 使用当前档位的采集FPS（后置240fps，前置120fps）
                            self.startCaptureWithDevice(camera, width: self.fixedCaptureWidth, height: self.fixedCaptureHeight, fps: self.currentCaptureFPS)
                        }
    ...
    private func startCaptureWithDevice(_ device: AVCaptureDevice, width: Int, height: Int, fps: Int) {
        ...
        // 🔥 固定采集 1280×960 4:3 横屏（支持高帧率240/120fps）
        ...
        // 分辨率优先：先选最接近 1920x1080 的，分辨率相同时选最高FPS的，同等条件优先 420f（Full Range）
        guard let best = allFormats.sorted(by: { f0, f1 in
            ...
            // FPS也相同时，优先选择 420f（Full Range，更好画质）
            let pixelFormat0 = CMFormatDescriptionGetMediaSubType(f0.formatDescription)
            let pixelFormat1 = CMFormatDescriptionGetMediaSubType(f1.formatDescription)
            // 420f = kCVPixelFormatType_420YpCbCr8BiPlanarFullRange = 0x34323066 = '420f'
            let is420f_0 = (pixelFormat0 == 0x34323066)
            let is420f_1 = (pixelFormat1 == 0x34323066)
            if is420f_0 != is420f_1 { return is420f_0 }  // 420f 优先
        ...
        // 🔥 打印选中的像素格式
        let selectedPixelFormat = CMFormatDescriptionGetMediaSubType(best.formatDescription)
        let formatStr = String(format: "%c%c%c%c",
                               (selectedPixelFormat >> 24) & 0xFF,
                               (selectedPixelFormat >> 16) & 0xFF,
                               (selectedPixelFormat >> 8) & 0xFF,
                               selectedPixelFormat & 0xFF)
        print("   📹 像素格式: \(formatStr) (\(selectedPixelFormat == 0x34323066 ? "Full Range 更好画质" : "Video Range"))")
        ...
        print("   🎯 当前格式FOV=\(String(format: "%.1f", best.videoFieldOfView))° Binned=\(best.isVideoBinned)")
    ...
    func applyMountTransform() {
        ...
        // ✅ 使用配置的方向，不强制
        let want: AVCaptureVideoOrientation = mountOrientation.avOrientation
        ...
        if let device = (session.inputs.first as? AVCaptureDeviceInput)?.device {
            ...
            print("🧭 采集层固定横屏(landscapeRight), 输出16:9, 已应用连接数=\(applied)")
        }
    }

    // 对外接口（UI 调用）- 输出始终16:9，此接口仅影响镜像
   ...
   private let ADAPT_MIN_INTERVAL_SEC: TimeInterval = 8
    ...
    // 🔥 固定采集分辨率 4:3（帧率优先，横屏锁定，通过 scaleResolutionDownBy 缩放输出）
    // 1280x960 是 4:3 比例，且支持高帧率（后置240fps，前置120fps）
    private let fixedCaptureWidth: Int = 1280
    private let fixedCaptureHeight: Int = 960
    ...
    func startPublish(initialProfile: LadderProfile? = nil) {
        ...
        if let pushTrack = localVideoTrack, capturer != nil {
            ...
            if let camera = initialCamera {
                // 使用当前档位的采集FPS（后置240fps，前置120fps）
                startCaptureWithDevice(camera, width: fixedCaptureWidth, height: fixedCaptureHeight, fps: currentCaptureFPS)
            }
        }
    ...
    func toggleCamera() {
        ...
        // 🔥 固定采集 1280×960 4:3 横屏（通过 scaleResolutionDownBy 缩放输出）
        let targetWidth = fixedCaptureWidth
        let targetHeight = fixedCaptureHeight
        
        print("🔄 [toggleCamera] 固定采集分辨率: \(targetWidth)x\(targetHeight)")
    ...
    func recapture(width: Int, height: Int, fps: Int) {
        // 默认使用固定采集分辨率
        recaptureWithResolution(targetWidth: fixedCaptureWidth, targetHeight: fixedCaptureHeight, targetFps: nil)
    }
    
    // 🔥 指定分辨率的重采集（用于前置 standard 档位切换到 640x480@60fps）
    func recaptureWithResolution(targetWidth: Int, targetHeight: Int, targetFps: Int?) {
        // 选择摄像头（沿用当前，若无则取后置）
        ...
        // 🔥 打印当前档位和摄像头信息
        print("🔄 [recaptureWithResolution] 目标采集=\(targetWidth)x\(targetHeight), 摄像头=\(dev.position == .back ? "后置" : "前置")")
        ...
        // ✅ 采集FPS：如果指定了targetFps则使用，否则用设备最大值
           let useFps = (targetFps != nil) ? min(targetFps!, maxFps) : maxFps
           currentCaptureFPS = useFps  // 记录采集FPS
           print("🎯 重采集采集FPS=\(useFps) (\(targetFps != nil ? "指定" : "设备最大")FPS, \(dev.position == .back ? "后置" : "前置"))")
