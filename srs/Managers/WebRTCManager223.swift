...
final class WebRTCManager: NSObject, ObservableObject {
   ...
   private let ADAPT_MIN_INTERVAL_SEC: TimeInterval = 8
    ...
    func startPublish(initialProfile: LadderProfile? = nil) {
        ...
        cfg.iceBackupCandidatePairPingInterval = 2000  // 🔥 2秒备用候选对ping（更快切换）
        cfg.iceCandidatePoolSize = 2  // 🔥 预分配ICE候选池
    ...
    private func mungeH264ForSRS(_ sdp: String) -> String {
        ...
        for i in 0..<lines.count {
            ...
            if l.lowercased().hasPrefix("a=fmtp:\(pt)") {
                ...
                // 🔥 WebRTC自适应码率：不设置固定码率，让WebRTC根据网络自动调整
                // 移除 x-google-min-bitrate, x-google-max-bitrate, x-google-start-bitrate
                // 让编码器根据网络带宽自适应调整码率
                dict.removeValue(forKey: "x-google-start-bitrate")
                dict.removeValue(forKey: "x-google-min-bitrate")
                dict.removeValue(forKey: "x-google-max-bitrate")
                
                print("🌐 SDP自适应码率: 由WebRTC根据网络状况自动调整")
    ...
    */
    ...
    func setMaxBitrateKbps(_ kbps: Int) {
        ...
        // 🌐 WebRTC自适应码率：不设置固定的min/max，让WebRTC根据网络自动调整
        // 只设置一个参考最大值，但不强制限制
        let maxBps = kbps * 1000
        params.encodings[0].maxBitrateBps = NSNumber(value: maxBps)  // 仅作为上限参考
        params.encodings[0].minBitrateBps = nil  // 🔥 不设置最小值，允许WebRTC自由向下调整
        ...
        if let encoding = verifyParams.encodings.first {
            ...
            print("🌐 自适应码率设置: 最大参考=\(kbps)kbps (WebRTC根据网络自动调整)")
    ...
    private func enforceBitrateImmediately() {
        ...
        // 计算输出分辨率
        let outputW = Int(Double(currentCaptureWidth) / scaleDown2)
        let outputH = Int(Double(currentCaptureHeight) / scaleDown2)
        print("🌐 自适应码率已设置: 最大参考=\(maxBps/1000)kbps, WebRTC=\(webrtcFps)fps, 采集=\(currentCaptureWidth)x\(currentCaptureHeight) → 输出=\(outputW)x\(outputH) (scale=\(scaleDown2))")
    }
    
    // 🌐 自适应码率模式：不再周期性强制码率
    // 已禁用此功能，让WebRTC根据网络状况自动调整码率
    private func startBitrateEnforcement() {
        // 🌐 自适应码率：不启动定时器，让WebRTC自由调整
        print("🌐 自适应码率模式：不启动周期性码率强制定时器")
    }
