...
final class WebRTCManager: NSObject, ObservableObject {
   ...
   private func calculateLadderForDevice(_ device: AVCaptureDevice) {
        // 🔥🔥 所有设备统一采集 1920x1440 (4:3)
        // ultra (16:9) 是特殊档位，需要单独采集 1280x720
        // 🔥 iPhone 15+ 超高清(p4k) 通过 scaleDown 缩放到 1920x1080
        
        let needP4kScale = isIPhone15OrNewer()
        
        // 🔥 超高清档位：iPhone 15+ 采集1920x1440 → 缩放输出1920x1080 (scaleDown=4/3)
        //              iPhone 13/14 采集1920x1440 → 原始输出1920x1440 (scaleDown=1.0)
        let p4kPreset: LadderPreset
        if needP4kScale {
            p4kPreset = LadderPreset(width: 1920, height: 1080, fps: 60, maxKbps: 5500, maxPushFps: 60, scaleDown: 4.0/3.0)
        } else {
        ...
        let standardPreset = LadderPreset(width: 768,  height: 576,  fps: 60, maxKbps: 2500, maxPushFps: 60, scaleDown: 2.5)
        
        let p4kInfo = needP4kScale ? "1920x1080(缩放自1440)" : "1920x1440(原始)"
        ...
        if device.position == .back {
            ...
            print("📐 前置摄像头 - 档位配置（统一采集1920x1440）：")
            print("   超高清(p4k)   = \(p4kPreset.width)x\(p4kPreset.height) @60fps → 5500kbps [\(p4kInfo)]")
   ...
   private let ADAPT_MIN_INTERVAL_SEC: TimeInterval = 8
    ...
    func applyProfileBitrateOnly(_ p: LadderProfile) {
        ...
        // - 其他档位统一采集 1920x1440 (4:3)，超高清 iPhone15+ 通过 scaleDown 缩放输出
        let isOldUltra = (oldProfile == .ultra)
        let isNewUltra = (p == .ultra)
        let needRecapture = (isOldUltra != isNewUltra)  // 只有在 ultra <-> 非ultra 切换时才重采集
        
        if needRecapture {
            // 🔥 需要重采集（ultra ↔ 非ultra 切换）
            print("   🔄 需要重采集（ultra ↔ 非ultra 切换）")
            if isNewUltra {
                // 切换到 ultra (16:9)
                recaptureWithResolution(width: 1280, height: 720, fps: preset.fps)
            } else {
                // 切换到非ultra档位，统一采集 1920x1440 (4:3)
    ...
    */
    ...
    /// 🔥 获取档位的实际采集分辨率
    /// 所有设备统一：非ultra采集 1920x1440 (4:3)，ultra采集 1280x720 (16:9)
    /// 超高清(p4k) iPhone 15+ 通过编码器 scaleDown 缩放输出到 1920x1080，采集不变
