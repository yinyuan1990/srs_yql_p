...
// MARK: - iPhone型号检测（用于区分编码能力）
fileprivate func isIPhone16OrNewer() -> Bool {
    ...
    if identifier.hasPrefix("iPhone") {
        ...
        if let commaIndex = numPart.firstIndex(of: ","),
           let majorVersion = Int(numPart[..<commaIndex]) {
            // iPhone 16 = iPhone17,x，所以 majorVersion >= 17 就是 iPhone 16+
...
final class WebRTCManager: NSObject, ObservableObject {
   ...
   private func calculateLadderForDevice(_ device: AVCaptureDevice) {
        ...
        // 
        // ⚠️ iPhone 16+ 的 A18 编码器在高分辨率下需要更高码率，否则容易出现马赛克
        // 解决方案：iPhone 16+ p4k 输出 1600x1200，其他设备输出 1920x1440
        
        let isiPhone16Plus = isIPhone16OrNewer()
        
        // p4k 档位配置（根据设备型号区分）
        let p4kPreset: LadderPreset
        if isiPhone16Plus {
            // iPhone 16+：缩放到 1600x1200（标准 UXGA 4:3，兼容性最好）
            p4kPreset = LadderPreset(width: 1600, height: 1200, fps: 60, maxKbps: 5500, maxPushFps: 60, scaleDown: 1.2)
        } else {
            // iPhone 15 及以下：输出原始 1920x1440
            p4kPreset = LadderPreset(width: 1920, height: 1440, fps: 60, maxKbps: 5500, maxPushFps: 60, scaleDown: 1.0)
        }
        
        if device.position == .back {
            // 🎥 后置摄像头（采集 1920x1440，通过 scaleDown 缩放输出）
            currentLadder = [
                .p4k:      p4kPreset,
                .ultra:    LadderPreset(width: 1280, height: 720, fps: 240, maxKbps: 3500, maxPushFps: 60, scaleDown: 1.0),      // 16:9 单独采集
                .high:     LadderPreset(width: 1440, height: 1080, fps: 60, maxKbps: 3500, maxPushFps: 60, scaleDown: 4.0/3.0),  // 4:3 缩放到1440x1080
                .standard: LadderPreset(width: 768,  height: 576, fps: 60, maxKbps: 2500, maxPushFps: 60, scaleDown: 2.5)        // 4:3 缩放到768x576
            ]
            let p4kInfo = isiPhone16Plus ? "1600x1200 (iPhone16+ UXGA)" : "1920x1440"
            print("📐 后置摄像头 - 档位配置（\(isiPhone16Plus ? "iPhone16+" : "iPhone15-")）：")
            print("   超高清(p4k)   = \(p4kInfo) @60fps → 5500kbps")
            ...
            // 📱 前置摄像头（采集 1920x1440，通过 scaleDown 缩放输出）
            currentLadder = [
                .p4k:      p4kPreset,
            ...
            let p4kInfo = isiPhone16Plus ? "1600x1200 (iPhone16+ UXGA)" : "1920x1440"
            print("📐 前置摄像头 - 档位配置（\(isiPhone16Plus ? "iPhone16+" : "iPhone15-")）：")
            print("   超高清(p4k)   = \(p4kInfo) @60fps → 5500kbps")
    ...
    func setAverageOutputFPS22(_ fps: Int) {
