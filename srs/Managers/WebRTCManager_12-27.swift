...
final class WebRTCManager: NSObject, ObservableObject {
   ...
   private func calculateLadderForDevice(_ device: AVCaptureDevice) {
    ...
    /// - Parameter forceApply: 是否强制应用（即使值相同）
    func setCaptureFrameRate(percentage: Int, forceApply: Bool = false) {
        ...
        // 🔥 如果目标FPS有变化，或者强制应用，重新配置采集
        // 同时检查当前采集FPS是否与目标一致（防止状态不同步）
        let needsApply = (oldTargetFps != newTargetFps) || forceApply || (currentCaptureFPS != newTargetFps)
        
        if needsApply {
            if currentCaptureFPS != newTargetFps {
                print("   ⚠️ 当前采集FPS(\(currentCaptureFPS)) 与目标(\(newTargetFps))不一致，强制应用")
            }
            applyCaptureFPSChange(targetFps: newTargetFps)
        } else {
            print("   ✅ 采集FPS已是目标值，无需重新配置")
        }
    }
    ...
    func applyThinRemoteConfigInit(_ cfg: ThinRemoteConfig) {
            ...
            if let cj = cfg.cjfps {
                ...
                // 🔥 调用 setCaptureFrameRate 确保立即生效（如果摄像头已启动）
                setCaptureFrameRate(percentage: cj)
                }
   ...
   private let ADAPT_MIN_INTERVAL_SEC: TimeInterval = 8
    ...
    // 🔥 固定采集分辨率（16:9 支持高帧率120/240fps）
    // 注意：此设备所有格式 FOV 相同（73.7°），4:3 不会多看上下
