...
final class WebRTCManager: NSObject, ObservableObject {
   ...
   private func calculateLadderForDevice(_ device: AVCaptureDevice) {
        // 🔥🔥 全系 4:3 采集（1920x1440），通过 scaleResolutionDownBy 缩放输出
        // scale=1:   1920x1440 (4K档)
        // scale=1.5: 1280x960  (超清档)
        // scale=2:   960x720   (高清档)
        // scale=3:   640x480   (标清档)
        
        if device.position == .back {
            // 🎥 后置摄像头：4档固定（采集 1920x1440 @60fps，4:3 最高只支持 60fps）
            currentLadder = [
                .p4k:      LadderPreset(width: 1920, height: 1440, fps: 60, maxKbps: 4500, maxPushFps: 60),   // scale=1
                .ultra:    LadderPreset(width: 1280, height: 960,  fps: 60, maxKbps: 3000, maxPushFps: 60),   // scale=1.5
                .high:     LadderPreset(width: 960,  height: 720,  fps: 60, maxKbps: 2000, maxPushFps: 60),   // scale=2
                .standard: LadderPreset(width: 640,  height: 480,  fps: 60, maxKbps: 1500, maxPushFps: 60)    // scale=3
            ]
            print("📐 后置摄像头 - 全系4:3配置（1920x1440采集）：")
            print("   4K    = 1920x1440 (scale=1)   → 4500kbps")
            print("   超清  = 1280x960  (scale=1.5) → 3000kbps")
            print("   高清  = 960x720   (scale=2)   → 2000kbps")
            print("   标清  = 640x480   (scale=3)   → 1500kbps")
        } else {
            // 📱 前置摄像头：4档固定（采集 1920x1440 @60fps，4:3 最高只支持 60fps）
            currentLadder = [
                .p4k:      LadderPreset(width: 1920, height: 1440, fps: 60, maxKbps: 4500, maxPushFps: 60),   // scale=1
                .ultra:    LadderPreset(width: 1280, height: 960,  fps: 60, maxKbps: 3000, maxPushFps: 60),   // scale=1.5
                .high:     LadderPreset(width: 960,  height: 720,  fps: 60, maxKbps: 1500, maxPushFps: 60),   // scale=2
                .standard: LadderPreset(width: 640,  height: 480,  fps: 60, maxKbps: 1200, maxPushFps: 60)    // scale=3
            ]
            print("📐 前置摄像头 - 全系4:3配置（1920x1440采集）：")
            print("   4K    = 1920x1440 (scale=1)   → 4500kbps")
            print("   超清  = 1280x960  (scale=1.5) → 3000kbps")
            print("   高清  = 960x720   (scale=2)   → 1500kbps")
            print("   标清  = 640x480   (scale=3)   → 1200kbps")
        }
    }
    ...
    func startPreviewIfNeeded(initialProfile: LadderProfile? = nil) {
        ...
        AVCaptureDevice.requestAccess(for: .video) { granted in
                    DispatchQueue.main.async {
                        ...
                        // ✅ 全系 4:3 采集（1920x1440）
                        if let camera = initialCamera {
                            print("🎬 [初始化-预览] 采集分辨率: \(self.fixedCaptureWidth)x\(self.fixedCaptureHeight) (4:3)")
   ...
   private let ADAPT_MIN_INTERVAL_SEC: TimeInterval = 8
    ...
    // 🔥 固定采集分辨率（4:3 比例）
    // 全系使用 1920x1440 (4:3) 采集，配合智能快门减少拖影
    private let fixedCaptureWidth: Int = 1920
    private let fixedCaptureHeight: Int = 1440
    ...
    func startPublish(initialProfile: LadderProfile? = nil) {
        ...
        if let pushTrack = localVideoTrack, capturer != nil {
            ...
            // ✅ 全系 4:3 采集（1920x1440）
            if let camera = initialCamera {
                print("🎬 [初始化-推流] 采集分辨率: \(fixedCaptureWidth)x\(fixedCaptureHeight) (4:3)")
    ...
    func toggleCamera() {
        ...
        print("🔄 [toggleCamera] 采集分辨率: \(targetWidth)x\(targetHeight) (4:3)")
    ...
    func getResolutionScaleForProfile(_ profile: LadderProfile) -> Double {
        ...
        // 🔥🔥 全系 4:3 采集（1920x1440），缩放到目标分辨率
        // 4K: scale=1 (1920x1440)
        // 超清: scale=1.5 (1280x960)
        // 高清: scale=2 (960x720)
        // 标清: scale=3 (640x480)
